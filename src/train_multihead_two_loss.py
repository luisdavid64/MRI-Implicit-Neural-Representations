...
from models.networks import WIRE, Positional_Encoder, FFN, SIREN, MultiHeadWrapperLossEnsemble
...
def train(opts):
    ...
    model = MultiHeadWrapperLossEnsemble(
        backbone=model_back,
        no_heads=no_models,
    ...
    loss_fn_center = torch.nn.MSELoss()
    loss_fn_periphery = MSLELoss()
    ...
    all_dists = dataset.coords[:,-1].unsqueeze(dim=-1)
    dataset.image = scale_space(stats, dataset.image, all_dists, part_radii).reshape(C*H*W,S)
    ...
    train_image = scale_space(stats_rec, train_image, all_dists, part_radii).reshape(C*H*W,S)
    ...
    for epoch in range(max_epoch):
        ...
        for it, (coords, gt, dist_to_center) in enumerate(data_loader):
            ...
            for i in range(no_models):
                ...
                if ind[0].numel():
                    gt_local = gt[ind]
                    for idx in range(0,len(layer_outs),2):
                        # Get gradients for final layers, and scale if target
                        # Make hyperparam is better probs
                        out_local_center = layer_outs[idx][ind]
                        out_local_periphery = layer_outs[idx+1][ind]
                        # Renormalize with 1!
                        multiplier = (1 if id == i else 0.00000001)
                        if config["loss"] in ["HDR", "LSL", "FFL", "tanh"]:
                            loss, _ = loss_fn_center(out_local_center, gt_local, coords.to(device))
                            train_loss += multiplier * loss
                        else:
                            train_loss += multiplier * loss_fn_center(out_local_periphery, gt_local)
                        if config["loss"] in ["HDR", "LSL", "FFL", "tanh"]:
                            loss, _ = loss_fn_periphery(out_local_periphery, gt_local, coords.to(device))
                            train_loss += multiplier * loss
                        else:
                            train_loss += multiplier * loss_fn_periphery(out_local_periphery, gt_local)
            if config["loss"] in ["HDR", "LSL", "FFL", "tanh"]:
                loss, _ = loss_fn_center(train_output, gt, coords.to(device))
                train_loss += loss
            else:
                train_loss +=  loss_fn_center(train_output, gt)
            optim.zero_grad()
            train_loss.backward()
            optim.step()
        ...
        if (epoch + 1) % config['val_epoch'] == 0:
            ...
            with torch.no_grad():
                ...
                for it, (coords, gt, dist_to_center) in tqdm(enumerate(val_loader), total=len(val_loader)):
                    ...
                    test_loss = 0
                    if config["loss"] in ["HDR", "LSL", "FFL", "tanh"]:
                        test_loss, _ = loss_fn_center(test_output, gt, coords.to(device))
                    else:
                        test_loss = loss_fn_center(test_output, gt)
                    test_running_loss += test_loss.item()
                    im_recon[it*bs:(it+1)*bs, :] = test_output.detach().cpu()
            im_recon = scale_space(stats_rec, im_recon, all_dists, part_radii)
