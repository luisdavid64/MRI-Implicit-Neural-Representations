...
def train(opts):
    ...
    params_1 = []
    if model.backbone:
        params_1 = params_1 + list(model.backbone.parameters())
    for i in range(no_models):
        params_1 = params_1 + list(model.heads[i].parameters())
    params_2 = list(model.weighted_avg.parameters())
    # Setup optimizer
    if config['optimizer'] == 'Adam':
        # Make sure all params optimized
        optim_1 = torch.optim.Adam(params_1, lr=config['lr'], betas=(config['beta1'], config['beta2']), weight_decay=config['weight_decay'])
        optim_2 = torch.optim.Adam(params_2, lr=config['lr'], betas=(config['beta1'], config['beta2']), weight_decay=config['weight_decay'])
    ...
    scheduler_1 = LambdaLR(optim[0], lambda x: 0.2**min(x/max_epoch, 1))
    scheduler_2 = LambdaLR(optim[1], lambda x: 0.2**min(x/max_epoch, 1))
    ...
    for epoch in range(max_epoch):
        ...
        for it, (coords, gt, dist_to_center) in enumerate(data_loader):
            ...
            if config["loss"] in ["HDR", "LSL", "FFL", "tanh"]:
                ...
                out_loss += 0.5 * loss_fn(train_output, gt)
            optim[1].zero_grad()
